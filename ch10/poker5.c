...
num_in_rank and num_in_suit; checks for bad cards and duplicate cards. */
void read_cards(void)
{
...
“ace-low” straights (ace, two, three, four, five). */
    ...
    for (; rank < NUM_RANKS && num_in_rank[rank] > 0; rank++)
        num_consec++;
    if (num_consec == NUM_CARDS || (num_in_rank[12] > 0 && num_in_rank[0] > 0 && num_in_rank[1] > 0 && num_in_rank[2] > 0 && num_in_rank[3] > 0)) 
    {
        straight = true;
        return;
    }
...
values of the external variables straight, flush, four, three, and pairs. */
void print_result(void)
{
    if (straight && flush)
